...
const timeToClose = () => {
    ...
    const now = dayjs();
    const closing = dayjs().millisecond(0).second(0).minute(0).hour(utcClosingHour);
    return Math.max(closing.diff(now, 'second'), 0);
  };
...
const ClosingSoon = () => {
  ...
  return (
    secondsToClose < 1800 ?
      ...
      <h1> closing soon: {minutes.toString().padStart(2, '0')}:{seconds.toString().padStart(2, '0')}</h1>
      </>
      : <></>
  );
};
